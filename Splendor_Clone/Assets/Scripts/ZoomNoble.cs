...
public class ZoomNoble : MonoBehaviour {
    ...
    public GameObject singleCardsCost;

    public void setValues(NobleDisplay n) {
        this.name = n.name;
    ...
    public void clearValues() {
        for (int i=0; i<cardsCosts.transform.childCount; i++) {
            Destroy(cardsCosts.transform.GetChild(i).gameObject);
        }
    }
    ...
    private void SetZoomCardData() {
        ...
        if (ruby > 0) {
            GameObject temp = Instantiate(singleCardsCost);
